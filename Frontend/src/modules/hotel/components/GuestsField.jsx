...
export default function GuestsField({
  ...
  return (
    <div
      ...
      className={`
        relative border border-gray-300 rounded px-3 cursor-pointer
        hover:border-[#0077b6] transition-all bg-white h-full
        ${compact ? "py-1" : "py-2"}
      `}
    >
      {/* LABEL */}
      ...
      <div
        className={`
          flex items-center justify-between h-full px-1 md:px-2
      ...
      {open && (
        ...
        >
                ...
                {(value?.childAges || []).map((age, i) => (
                  ...
                  >
                    ...
                    <select
                      ...
                      className="border border-gray-400 rounded-md px-2 py-1 text-sm bg-white text-gray-900"
                    >
                      {Array.from(
                        { length: 17 },
                        (_, i) => i + 1
          ...
          <Button
            onClick={handleApply}
            className="mt-6 w-full text-white !py-5 rounded-xl font-semibold hover:!bg-[#005f8f]"
            style={{
              background:
