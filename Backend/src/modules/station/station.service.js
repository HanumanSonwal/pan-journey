...
export const getStations = async ({
  ...
  const cacheKey = `stations:${search || "all"}:${status}:${page}:${limit}:${sortBy}:${order}:${adminView}`;
  ...
  const [data, total] = await Promise.all([
    Station.find(query).sort(sortOption).skip(skip).limit(limit).lean(),
...
export const bulkDeleteStations = async (ids) => {
  if (!ids.length) throw new ApiError(400, "No IDs provided");

  await Station.updateMany({ _id: { $in: ids } }, { $set: { status: false } });
...
export const bulkUpdateStatus = async (ids, status) => {
  ...
  await Station.updateMany({ _id: { $in: ids } }, { $set: { status } });
